...
template<typename T>
bool IsSubset(const set<T> &s1, const set<T> &s2) {
    for (auto &t: s2)
        if (!s1.count(t))
            return false;
...
bool FindInStrVec(const vector<string> &vec, const string &s) {
...
template<typename T>
void UnionSet(set<T> &s1, set<T> &s2) {
        ...
        for (auto &symbol: t.next_symbol) {
            ...
            if (dfa_idx == -1) {
                ...
                for (auto item: items) {
                    ...
                    if (item.dot_pos < item.symbols.size() && !FindInStrVec(
                            item_set.next_symbol, item.symbols[item.dot_pos]))
...
// 判断两个集合是否有交集
bool HasIntersection(const set<string> &s1, const set<string> &s2) {
...
int GetReducedIdx(int V_idx, const vector<string> &P) {
...
void InitAction() {
    ...
    rep(i, 0, dfa_cnt) {
        ...
        bool has_intersection = !AreSetsDoNotIntersectEachOther(alphas_followB) && (
                                    Bs.size() == B_reduced_items.size());

        if (reduced_cnt > 0 && reduced_cnt < DFA[i].items.size() || reduced_cnt > 1 && reduced_cnt == DFA[i].items.
            size()) {
...
int main() {
    cout << "请输入文法定义的文件名称（包含文件后缀名）：";
    // cin >> grammar_filename;
    grammar_filename = "test/SLR_Grammar_3.txt";
    ...
    cout << "请输入待分析句子的文件名称（包含文件后缀名）：";
    // cin >> sentence_filename;
    sentence_filename = "test/SLR_Sentence_3_1.txt";
